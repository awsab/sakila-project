 ...
 * User     : ramup
 * Date     : 20/04/2026
 * Usage    : Shared HTTP request/response logging interceptor provided by awsab-framework.
 *            Automatically registered for all /api/** paths via {@link AwsabWebMvcAutoConfiguration}.
 *            Child services do NOT need to create their own LoggingInterceptor.
 * Since    : Version 1.0
 */
@Slf4j
public class AwsabLoggingInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle (HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (log.isDebugEnabled ()) {
            log.debug ("%n====== Request Begin ======%nURI    : {}%nMethod : {}%nAddr   : {}",
