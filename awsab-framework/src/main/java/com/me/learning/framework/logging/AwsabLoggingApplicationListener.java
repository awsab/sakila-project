 ...
 * User     : ramup
 * Date     : 15/04/2026
 * Usage    : Early application listener that enforces AWSAB structured logging
 * Since    : Version 1.0
 */
...
/**
 * Spring {@link ApplicationListener} that automatically configures AWSAB structured
 ...
 *   <li>Logstash TCP appender is skipped unless {@code awsab.logging.logstash-enabled=true}.</li>
 ...
 * {@code logback-spring.xml} that already uses {@link AwsabLogging.LogbackContextListener}),
 * this listener skips re-registration to avoid duplicate appenders.
 */
public class AwsabLoggingApplicationListener
        implements ApplicationListener<ApplicationEnvironmentPreparedEvent>, Ordered {

    private static final String PROP_JSON_ENABLED   = "awsab.logging.json-format-enabled";
    private static final String PROP_LOGSTASH       = "awsab.logging.logstash-enabled";
    ...
    private static final String DEFAULT_APP_NAME    = "awsab-app";
    ...
    public void onApplicationEvent(ApplicationEnvironmentPreparedEvent event) {
        ...
        if (rootLogger.getAppender(AwsabLogging.CONSOLE_APPENDER_NAME) != null) {
            return;
        }
        ...
        if (jsonEnabled) {
            AwsabLogging.addJSONAppender(loggerContext, fields);
        }
        if (logstashEnabled) {
            AwsabLogging.addLogstashAppender(loggerContext, fields);
        }
    }
